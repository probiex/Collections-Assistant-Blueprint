...
export function predictPayment(
  ...
  const lastReminder = invoice.last_reminder_date
    ? parseDate(invoice.last_reminder_date)
    : null;
  const history = invoice.payment_history.toLowerCase();

  let offsetDays = 14;
  let confidence: PredictionConfidence = "Medium";
  let rule = "typical account behavior";

  if (history.includes("always on time")) {
    offsetDays = 2;
    confidence = "High";
    rule = "the customer's consistent on-time payment history";
  } else if (history.includes("first time late")) {
    offsetDays = 9;
    confidence = invoice.relationship_years >= 2 ? "High" : "Medium";
    rule = "a first late payment, usually resolved within 7–10 days of follow-up";
  } else if (history.includes("occasionally late")) {
    offsetDays = 16;
    confidence = invoice.relationship_years >= 2 ? "Medium" : "Low";
    rule = "occasional late payments, typically resolved within 14–18 days of follow-up";
  } else if (history.includes("chronic late")) {
    offsetDays = 30;
    confidence = "Low";
    rule = "a chronic late-payment pattern, typically requiring 25–35 days after follow-up";
  }

  const anchor =
    history.includes("always on time")
      ? due
      : new Date(Math.max((lastReminder ?? ref).getTime(), ref.getTime()));
  const predictedDate = new Date(anchor);
  predictedDate.setUTCDate(anchor.getUTCDate() + offsetDays);

  const minimumFutureDate = new Date(ref);
  minimumFutureDate.setUTCDate(ref.getUTCDate() + (history.includes("always on time") ? 2 : 1));
  if (predictedDate < minimumFutureDate) {
    predictedDate.setTime(minimumFutureDate.getTime());
  }

  const daysFromNow = Math.round(
    (predictedDate.getTime() - ref.getTime()) / (1000 * 60 * 60 * 24)
  );
  const explanation = `Estimated from ${rule}. Confidence also reflects ${invoice.relationship_years} year${invoice.relationship_years === 1 ? "" : "s"} of relationship history and ${invoice.reminders_sent} prior reminder${invoice.reminders_sent === 1 ? "" : "s"}.`;
